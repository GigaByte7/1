...
{
public:
    EnhancedPathRecorder() : nh_("~"), should_save_on_exit_(true), already_saved_(false)
    {
        // 设置静态实例指针
        instance_ = this;
        // 获取参数
        nh_.param<std::string>("map_frame", map_frame_, "prior_map");
        ...
        possible_map_frames_.push_back("world");
        ...
        nav_status_sub_ = nh_.subscribe("/move_base/status", 1,
    ...
    {
        // 如果导航未激活，不记录实际路径
        if (!navigation_active_ && save_actual_path_)
            return;
